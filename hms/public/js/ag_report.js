...
frappe.views.AgReport = class AgReport extends (
        ...
        df.onchange = () => {
          ...
          if (f.on_change) {
            ...
            if (this.prepared_report) {
              ...
              if (!f.value) {
                let cached =
                  frappe.query_reports[this.report_name].cached_filters;
                cached && delete cached[f.fieldname];
              }
              this.refresh();
            }
          }
  ...
  refresh() {
    this.toggle_message(true);
    let cached = frappe.query_reports[this.report_name].cached_filters || {};
    let filters = Object.assign(cached, this.get_filter_values(true));
  ...
  render_datatable() {
    ...
    let always_recreate =
      (this.datatable &&
        ...
        this.gridOptions.context.always_recreate) ||
      false;
  ...
  prepare_columns(columns) {
    return columns.map((column) => {
      ...
      // fix column filters
      column.filter =
        agfilter_for_fieldtype[column.fieldtype] || "agTextColumnFilter";
  ...
  export_report() {
    ...
    this.export_dialog = frappe.prompt(
      [
        {
          ...
          reqd: 1,
          onchange: () => {
            this.export_dialog.set_df_property(
              "with_indentation",
              "hidden",
      ...
      __("Export Report: " + this.report_name),
      __("Download")
    );
  }
  ...
  setup_report_wrapper() {
    ...
    $(
      `<div class="row" style="padding:3px 25px;">
        <div class="col-md-12 text-muted text-right small ag-header-message"></div>
      </div>`
  ...
  show_footer_message() {
    ...
    $(document.querySelector(".ag-header-message")).html(
      __("{0} records", [(this.data || []).length])
    );

    this.page.footer
      .removeClass("hide")
      .addClass("text-muted col-md-12")
      .html(
  ...
  get_selected_rows_after_filter(raise_error, only_selected) {
    ...
    if (raise_error && !doclist.length > 0) {
      frappe.throw(raise_error);
    }
    return doclist;
  }
};
...
frappe.set_ag_license = function () {
  ...
  //     callback: function(r) {
  //       if (r.message) agGrid.LicenseManager.setLicenseKey(r.message);
  //     }
  //   });
};
...
frappe.set_pinned_totals = function (report, total_columns) {
  ...
  for (let c of gop.columnDefs) {
    if (total_columns.includes(c.colId || c.field)) {
      row[c.colId || c.field] = 0;
    } else {
      row[c.colId || c.field] = "";
    }
  }
