...
def get_data(filters=None):
    ...
    data = frappe.db.sql(
        """
        select 
            guest, f.customer, mobile, email, f.name, 
            date_format(f.check_in,'%%Y-%%m-%%d %%H:%%i') check_in, 
            date_format(f.check_out,'%%Y-%%m-%%d %%H:%%i') check_out,
            f.room_no, f.room_type, f.total_charges
        from 
            `tabRoom Guest Detail HMS` g
            inner join `tabRoom Folio HMS` f on f.name = g.parent and f.docstatus <> 2
        where 
            f.check_in >= %(from_date)s
        order by 
            f.check_in, guest
    """,
        filters,
