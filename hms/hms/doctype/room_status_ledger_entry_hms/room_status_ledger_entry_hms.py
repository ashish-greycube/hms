...
class RoomStatusLedgerEntryHMS(Document):
    def validate(self):
        ...
        for d in frappe.db.sql(
            ...
            (self.room_no, self.status),
        ):
            frappe.throw(
                "Entry for %s in %s status already exists."
                % (self.room_no, self.status)
            )
...
class update_room_status_ledger(object):
    ...
    def __init__(self, args, action):
        ...
        self.args = (
            ...
            and {
                ...
                "old_room_no": args.get("old_room_no"),
            }
            or {}
        )
    ...
    def check_in(self):
        ...
        exists_status = frappe.db.sql(
            """select status
        from `tabRoom Status Ledger Entry HMS` g
        where docstatus <> 2 and room_no = %s""",
            (self.args.get("room_no")),
        )
        if exists_status:
            frappe.throw(
                "Room %s is in %s status. Cannot Check In."
                % (self.args.get("room_no"), exists_status[0][0])
            )
    ...
    def move(self):
        new_room_no = self.args.get("room_no")
        # Check out of old room and set dirty
        self.args.update({"room_no": self.args.get("old_room_no")})
        self.check_out()
        # Check In into new room
        self.args.update({"room_no": new_room_no})
        self.check_in()
